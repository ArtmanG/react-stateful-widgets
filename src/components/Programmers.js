...
console.log(listOfAwesome[0].name)
...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programers[featuredId - 1].name
  };
      ...
      <div className='programmers'>
        {
          ...
          programers.map(dev =>
            <div key={dev.id}>
              {dev.name} <button onClick={() => {featuredId !== dev.id ? setFeaturedId(dev.id) : setFeaturedId(null)}}>Feature</button>
            </div>
          )
        }
      </div>
